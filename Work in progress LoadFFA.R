...
#setwd("~/Documents/cris")
LoadFFA <- function(filename, folder.out, TYPE = "PeakFQ_17C", colStandard = TRUE){ 
  
  # standardize column output names
  ls.colNames <- list("PEAKFQSA" = c("Ann. Exc. Prob.", "EMA Est.", "V[log(EMA)]","At-Site Est.","CI-Low","CI-High"), "PKFQ72_17C" = c("ANN_EXC_PROB",  "EMA_REG",	"EMA_STAT",	"LOG_VAR_REG",	"5PCT_CONF_REG",	"95PCT_CONF_REG"), "HEC_17B"    = c("Ordinate",  "FREQ",	"FLOW Computed Curve",	"FLOW Expected Prob Curve",	"FLOW 5Pct Conf",	"FLOW 95Pct Conf", "FLOW Observed (Median)",	"FLOW Historic Data",	"FLOW High Outlier",	"FLOW Low Outlier"), "PKFQ42_17B" = c("ANN_EXC_PROB",  "BULL17B_EST",	"SYSTEM_RECORD",	"EXPECTED_PROB",	"5PCT_CONF",	"95PCT_CONF"), "PARTDUR"    = c("Val","Exc"))
  colNames <- ls.colNames[TYPE]

  colsSwitch <- list(FlowCols   = c("EMA_REG", "FLOW Computed Curve", "BULL17B_EST","Val","EMA Est.","V[log(EMA)]","At-Site Est."), FreqCols   = c("Ann. Exc. Prob.","ANN_EXC_PROB","FREQ","Exc","Ann. Exc. Prob."), Flow05Cols = c("5PCT_CONF_REG", "5PCT_CONF","CI-Low"), Flow95Cols = c("95PCT_CONF_REG", "95PCT_CONF","CI-High"))

  colsOut <- c(FlowCols   = "Flow", FreqCols   = "Freq", Flow05Cols = c("Flow05"), Flow95Cols = c("Flow95"))
...
LoadFFA("03606500peaks.out","~/Documents/cris/","PEAKFQSA")
#
